...
class BERTopic:
    ...
    def topics_over_time(
        ...
        documents = pd.DataFrame(
            {"Document": docs, "Topic": topics, "Timestamps": timestamps}
        )
    ...
    def topics_per_class(
        ...
        for index, class_ in tqdm(enumerate(set(classes)), disable=not self.verbose):
            ...
            if global_tuning:
                c_tf_idf = normalize(c_tf_idf, axis=1, norm="l1", copy=False)
                c_tf_idf = (
                    global_c_tf_idf[documents_per_topic.Topic.values + 1] + c_tf_idf
                ) / 2.0
    ...
    def visualize_topics(
        ...
        return plotting.visualize_topics(
            self, topics=topics, top_n_topics=top_n_topics, width=width, height=height
        )
    ...
    def visualize_term_rank(
        ...
        return plotting.visualize_term_rank(
            self, topics=topics, log_scale=log_scale, width=width, height=height
        )
    ...
    def _reduce_dimensionality(
        ...
        """
        if not isinstance(self.umap_model, TSNE):
    ...
    def _save_representative_docs(self, documents: pd.DataFrame):
        ...
        for topic in documents["Topic"].unique():
            if topic != -1:
                ...
                representative_docs[topic] = list(
                    np.random.choice(result, 3, replace=False).astype(int)
                )

        # Convert indices to documents
        self.representative_docs = {
            topic: [documents.iloc[doc_id].Document for doc_id in doc_ids]
            for topic, doc_ids in representative_docs.items()
        }
    ...
    def _map_representative_docs(self, original_topics: bool = False):
        ...
        updated_representative_docs = {
            mappings[old_topic]: [] for old_topic, _ in representative_docs.items()
        }
    ...
    def _c_tf_idf(
        self, documents_per_topic: pd.DataFrame, fit: bool = True
    ...
    def _extract_words_per_topic(
        self, words: List[str], c_tf_idf: csr_matrix = None, labels: List[int] = None
        ...
        if self.diversity is not None:
            if self.embedding_model is not None:
                ...
                for topic, topic_words in topics.items():
                    ...
                    topics[topic] = [
                        (word, value)
                        for word, value in topics[topic]
    ...
    def _sort_mappings_by_frequency(self, documents: pd.DataFrame) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            self.topic_sizes.items(), columns=["Old_Topic", "Size"]
        ...
        if probabilities is not None:
            if len(probabilities.shape) == 2 and self.get_topic(-1):
                mapped_probabilities = np.zeros(
                    (probabilities.shape[0], len(set(mappings.values())) - 1)
                )
